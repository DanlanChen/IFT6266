...
scaled_stream = MinimumImageDimensions(stream,(64,64),which_sources=('image_features',))

#ur images are of different sizes, so we'll use a Fuel transformer
# to take random crops of size (32 x 32) from each image
#before cropping, we need to rescale all images to 64*64

cropped_stream = RandomFixedSizeCrop(
    scaled_stream, (32, 32), which_sources=('image_features',))
...
loss.name = 'loss'
extensions = [TrainingDataMonitoring([loss],every_n_batches = 1),Printing(every_n_batches = 1),FinishAfter(after_n_epochs=8)]
