...
def create_model(nb_conv,img_rows,img_cols,nb_pool,file):
	model = Sequential()
	#(a) 
	model.add(Convolution2D(30, 4, 4,input_shape=( 3,img_rows, img_cols)))
	...
	#(f) 
	model.add(Convolution2D(90, 2, 2))
	model.add(Activation('relu'))

	model.add(Convolution2D(120, 2, 2))
	model.add(Activation('relu'))
	model.add(Dropout(0.2))
	...
	model.add(Convolution2D(250, 2, 2))
	model.add(Activation('relu'))
	model.add(Dropout(0.2))
...
#	print (data_x)
	...
	x = data_x[p_3]
	y = data_y[p_3]
	f1,f2,f3,f4,f5 = "model_04_18_8.json", "weights_04_18_8.hdf5","prediction_model_04_18_8.csv","08_model_train_1.json","08_model_train_2.json"  
